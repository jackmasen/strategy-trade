...
class MarketManager:
    ...
    def start(self, symbols: Optional[List[str]] = None) -> None:
        ...
        if self._primary_client:
            default_symbols = ["BTC", "ETH", "SOL", "XAU", "WTI", "SKHYNIX", "SNDK", "SAND", "HBAR"]
        ...
        if self._primary_client:
            all_syms = list(symbols or self._symbols_subscribed) or ["BTC", "ETH", "SOL", "XAU", "WTI", "SKHYNIX", "SNDK", "SAND", "HBAR"]
            try:
        ...
        while not self._stop_event.is_set():
            try:
                ...
                if not self._primary_client:
                    continue
                # 只处理已订阅 + 主用5品种
                syms = list(self._symbols_subscribed) or ["BTC", "ETH", "SOL", "XAU", "WTI", "SKHYNIX", "SNDK", "SAND", "HBAR"]
                for sym in syms:
                    try:
