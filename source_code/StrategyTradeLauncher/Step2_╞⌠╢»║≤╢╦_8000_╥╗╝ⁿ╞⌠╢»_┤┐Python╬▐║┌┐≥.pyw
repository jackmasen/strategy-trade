# -*- coding: utf-8 -*-
"""
策略交易系统 · 纯 Python 自包含后端启动器（.pyw 无黑框版本 · 新版）
- 目标：一键启动 FastAPI :8000，启动成功自动打开浏览器 /docs
...
# ============== 日志：写文件 + 控制台（.py 版本能看到打印；.pyw 只写文件，也不报错） ==============
...
def find_host_python() -> Optional[str]:
    if VENV_PY.exists():
        ...
        return str(VENV_PY)
    if EMBED_PY.exists():
        log_and_print(f"[Python] 找到 embeddable: {EMBED_PY}")
        return str(EMBED_PY)
    try:
...
def bootstrap_pip_in_venv(venv_py: str) -> None:
    ...
    if r2.returncode == 0:
        return
    log_and_print(f"[PIP] ensurepip exit={r2.returncode}，仍然继续尝试 pip install（系统可能会自己 bootstrap）")
    ...
    try:
        ...
        for m in modules_to_try:
            try:
                ...
                misses.append(m)
        if not misses:
            log_and_print("[PIP] 核心模块均已安装，跳过 pip install")
            return
    ...
    while time.time() < deadline:
        try:
            with urllib.request.urlopen(HEALTH_URL, timeout=2) as resp:
                body = resp.read()
            if resp.status == 200:
                log_and_print(f"[HEALTH] OK status=200 body={body[:200]}")
                try:
                    import webbrowser
                    webbrowser.open(DOCS_URL, new=2)
                    log_and_print(f"[BROWSER] 已打开 {DOCS_URL}")
                except Exception as e:
...
def start_uvicorn(venv_py: str) -> subprocess.Popen:
    ...
    log_and_print(f"[UVICORN] 启动成功 PID={p.pid}")
    return p
