# -*- coding: utf-8 -*-
"""
策略交易系统 · 纯 Python 自包含后端启动器（.py 有控制台版本 · 新版）
- 会显示一个黑色控制台窗口，实时打印启动过程，方便你判断是否在干活
...
- 【成功强信号】：控制台最后打印 [HEALTH] OK status=200 + 自动弹出浏览器打开 /docs
"""
import os
import re
import sys
...
def create_venv_if_needed(host_py: str) -> str:
    if VENV_PY.exists():
        return str(VENV_PY)
    log_and_print("[VENV] .venv 不存在，正在创建（首次启动需要几秒）...")
    t0 = time.time()
    r = subprocess.run(
    ...
    log_and_print(f"[VENV] 创建完成，耗时 {time.time()-t0:.1f}s")
    return str(VENV_PY)
...
def bootstrap_pip_in_venv(venv_py: str) -> None:
    ...
    if r2.returncode == 0:
        return
    log_and_print(f"[PIP] ensurepip exit={r2.returncode}，仍然继续 pip install（可能会自举）")
...
def pip_install_requirements(venv_py: str) -> None:
    try:
        ...
        if not misses:
            ...
            return
        log_and_print(f"[PIP] 缺以下模块，开始安装（首次启动耗时较久，请耐心等待）: {misses}")
    ...
    for cmd in commands:
        ...
        if r.returncode != 0:
            log_and_print(f"[PIP] FAIL 尾部:\n{last_out[-1200:]}")
            ok = False
            break
    if ok:
        return

    log_and_print("[PIP] 镜像失败，回退默认源重试...")
    ...
    if r_ok.returncode != 0:
        log_and_print(f"[PIP] 仍失败：无法 import fastapi/uvicorn/sqlalchemy。尾部:\n{last_out[-1500:]}")
...
def kill_port_8000() -> None:
    ...
    try:
        ...
        log_and_print(f"[PORT] 清理异常（忽略）: {e}")


def health_probe_and_open_browser(max_wait: int = 180) -> bool:
    log_and_print(f"[HEALTH] 开始轮询 {HEALTH_URL}（最多 {max_wait}s，直到 200 OK）")
    ...
    while time.time() < deadline:
        ...
        try:
            ...
            if resp.status == 200:
                ...
                try:
                    ...
                    print("=" * 60)
                    print("✅ 后端已启动成功！浏览器已自动打开 Swagger UI：")
                    print("   http://127.0.0.1:8000/docs")
                    print("👉 如果浏览器没弹，请手动复制上面链接到浏览器打开")
                    print("👉 现在可以在 TRAE 对话框里回复『已启动』让助手做连通性复测 + OpenPreview")
                    print("=" * 60)
                except Exception as e:
        ...
        time.sleep(2)
    log_and_print(f"[HEALTH] 超时未探测到 /health，最近一次错误: {last_err}")
    print("⚠️ 后端未正常启动，请查看日志：")
    print("  启动过程日志: " + str(LOG_LAUNCH))
    print("  后端运行日志: " + str(LOG_BACKEND))
    return False
...
def main() -> None:
    print("=" * 60)
    print("🚀 策略交易系统 · 纯 Python 一键启动后端 8000（有控制台版）")
    print("   项目根:", PROJECT_ROOT)
    print("=" * 60)
    log_and_print("=" * 60)
    log_and_print("策略交易系统 · 纯 Python 一键启动后端 8000（新版）")
    ...
    try:
        code = proc.wait()
        log_and_print(f"[UVICORN] 子进程退出 code={code}；请查看 _logs_backend.log 最后 200 行定位原因")
        print(f"\n⚠️ 后端进程退出 code={code}，查看 _logs_backend.log 最后 200 行定位原因")
    except KeyboardInterrupt:
        log_and_print("[UVICORN] 用户 Ctrl+C 停止")
        print("\n👋 已停止")
    input("\n按回车键退出...")
