...
class TradePosition(Base):
    ...
    trailing_mode = Column(Integer, default=0, comment="止损模式: 0-经典 1-ATR动态 2-分批止盈 3-保本止损 4-时间止损")
    ...
    # ATR动态止损
    atr_period = Column(Integer, default=14, comment="ATR周期")
    atr_multiplier = Column(Float, default=2.0, comment="ATR止损倍数")
    atr_value = Column(DECIMAL(18, 8), nullable=True, comment="当前ATR值")

    # 分批止盈
    tp1_price = Column(DECIMAL(18, 8), nullable=True, comment="第一档止盈价")
    tp1_ratio = Column(Float, default=0.3, comment="第一档平仓比例(0-1)")
    tp2_price = Column(DECIMAL(18, 8), nullable=True, comment="第二档止盈价")
    tp2_ratio = Column(Float, default=0.3, comment="第二档平仓比例(0-1)")
    tp3_price = Column(DECIMAL(18, 8), nullable=True, comment="第三档止盈价")
    tp3_ratio = Column(Float, default=0.4, comment="第三档平仓比例(0-1)")
    partial_tp_executed = Column(Integer, default=0, comment="已执行的分批止盈档位: 0=无 1=第一档 2=第二档 3=全部")

    # 保本止损
    breakeven_activation_pct = Column(Float, default=1.0, comment="激活保本止损的盈利百分比")
    breakeven_triggered = Column(Integer, default=0, comment="保本止损是否已触发: 0-否 1-是")

    # 时间止损
    time_stop_minutes = Column(Integer, default=0, comment="时间止损(分钟)，0=不启用")
