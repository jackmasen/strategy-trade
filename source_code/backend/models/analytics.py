...
class TradeAuditLog(Base):
    """交易审计日志 - 记录下单/平仓/风控完整上下文"""

    ACTION_ORDER_PLACED = 1     # 下单提交
    ACTION_ORDER_FILLED = 2    # 订单成交
    ACTION_ORDER_CANCELED = 3  # 订单撤销
    ACTION_ORDER_FAILED = 4    # 下单失败
    ACTION_POSITION_OPENED = 5 # 持仓开仓
    ACTION_POSITION_CLOSED = 6 # 持仓平仓
    ACTION_PARTIAL_CLOSED = 7  # 分批平仓
    ACTION_RISK_TRAILING = 8   # 移动止损触发
    ACTION_RISK_TP = 9         # 止盈触发
    ACTION_RISK_SL = 10        # 止损触发
    ACTION_RISK_DRAWDOWN = 11  # 回撤超限
    ACTION_RISK_DAILY = 12     # 日亏损超限
    ACTION_RISK_TIME_STOP = 13 # 时间止损
    ACTION_STRATEGY_EVOLVE = 14 # 策略进化

    __table_args__ = (
        Index("idx_audit_user_time", "user_id", "created_at"),
        Index("idx_audit_action", "action_type"),
        Index("idx_audit_pos", "position_id"),
    )
    ...
    action_type = Column(SmallInteger, index=True, comment="动作类型")
    action_desc = Column(String(255), default="", comment="动作描述")

    symbol = Column(String(32), nullable=True, comment="品种")
    side = Column(SmallInteger, nullable=True, comment="方向: 1-多 2-空")
    price = Column(DECIMAL(18, 8), default=0, comment="价格")
    quantity = Column(DECIMAL(18, 8), default=0, comment="数量")

    # 完整上下文快照
    market_snapshot = Column(JSON, default=dict, comment="市场数据快照(价格/成交量等)")
    position_snapshot = Column(JSON, default=dict, comment="持仓状态快照")
    strategy_snapshot = Column(JSON, default=dict, comment="策略参数快照")
    factor_scores = Column(JSON, default=dict, comment="7因子评分快照")
