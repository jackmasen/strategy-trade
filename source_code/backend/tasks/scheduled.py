...
from backend.models.analytics import DailyFinancialReport, RiskEventLog, TradeAuditLog
            ...
            for acc_id, positions in positions_by_account.items():
                ...
                try:
                    for pos in positions:
                        ...
                        # 2.5) Trailing Stop 移动止损（高级模式）
                        if pos.trailing_enabled == 1:
                            _mode = int(pos.trailing_mode or 0)
                            ...
                            profit_pct = 0.0
                            if pos.side == 1 and trailing_extreme > entry:
                                profit_pct = (trailing_extreme - entry) / entry * 100
                            elif pos.side == 2 and trailing_extreme > 0 and trailing_extreme < entry:
                                ...
                                if profit_pct >= activation:
                                    if pos.side == 1:
                                        ...
                                        if new_sl > current_sl:
                                            ...
                                            logger.debug(f"[Trailing-Classic] pos={pos.id} {pos.symbol} LONG SL {current_sl:.4f} -> {new_sl:.4f}")
                                    else:
                                        new_sl = trailing_extreme * (1 + distance / 100)
                                        current_sl = float(pos.sl_price or 0)
                                        if current_sl == 0 or new_sl < current_sl:
                                            pos.sl_price = Decimal(str(round(new_sl, 8)))
                                            logger.debug(f"[Trailing-Classic] pos={pos.id} {pos.symbol} SHORT SL {current_sl:.4f} -> {new_sl:.4f}")

                            # --- Mode 1: ATR动态止损 ---
                            elif _mode == 1:
                                try:
                                    atr_val = _calc_position_atr(client, pos)
                                    if atr_val > 0:
                                        pos.atr_value = Decimal(str(round(atr_val, 8)))
                                        atr_mult = float(pos.atr_multiplier or 2.0)
                                        if pos.side == 1:
                                            new_sl = trailing_extreme - atr_val * atr_mult
                                            current_sl = float(pos.sl_price or 0)
                                            if new_sl > current_sl and new_sl < mark_price:
                                                pos.sl_price = Decimal(str(round(new_sl, 8)))
                                                logger.debug(f"[Trailing-ATR] pos={pos.id} {pos.symbol} LONG SL {current_sl:.4f} -> {new_sl:.4f} ATR={atr_val:.4f}")
                                        else:
                                            new_sl = trailing_extreme + atr_val * atr_mult
                                            current_sl = float(pos.sl_price or 0)
                                            if (current_sl == 0 or new_sl < current_sl) and new_sl > mark_price:
                                                pos.sl_price = Decimal(str(round(new_sl, 8)))
                                                logger.debug(f"[Trailing-ATR] pos={pos.id} {pos.symbol} SHORT SL {current_sl:.4f} -> {new_sl:.4f} ATR={atr_val:.4f}")
                                except Exception as e:
                                    logger.warning(f"[Trailing-ATR] pos={pos.id} ATR计算失败: {e}")

                            # --- Mode 2: 分批止盈 ---
                            elif _mode == 2:
                                _executed = int(pos.partial_tp_executed or 0)
                                tp_levels = [
                                    (float(pos.tp1_price or 0), float(pos.tp1_ratio or 0.3)),
                                    (float(pos.tp2_price or 0), float(pos.tp2_ratio or 0.3)),
                                    (float(pos.tp3_price or 0), float(pos.tp3_ratio or 0.4)),
                                ]
                                for lvl_idx, (tp_p, tp_r) in enumerate(tp_levels):
                                    if tp_p <= 0 or _executed >= lvl_idx + 1:
                                        continue
                                    tp_triggered = False
                                    if pos.side == 1 and mark_price >= tp_p:
                                        tp_triggered = True
                                    elif pos.side == 2 and mark_price <= tp_p:
                                        tp_triggered = True
                                    if tp_triggered:
                                        try:
                                            partial_qty = float(pos.quantity_contracts) * tp_r
                                            result = _partial_close_position(db, pos, acc, tp_p, partial_qty, client)
                                            if result:
                                                pos.partial_tp_executed = lvl_idx + 1
                                                logger.info(f"[Trailing-PartialTP] pos={pos.id} {pos.symbol} 第{lvl_idx+1}档止盈执行 price={tp_p:.4f} qty={partial_qty:.4f}")
                                        except Exception as e:
                                            logger.warning(f"[Trailing-PartialTP] pos={pos.id} 第{lvl_idx+1}档执行失败: {e}")

                            # --- Mode 3: 保本止损 ---
                            elif _mode == 3:
                                be_activation = float(pos.breakeven_activation_pct or activation)
                                if profit_pct >= be_activation and int(pos.breakeven_triggered or 0) == 0:
                                    pos.sl_price = Decimal(str(round(entry, 8)))
                                    pos.breakeven_triggered = 1
                                    logger.info(f"[Trailing-Breakeven] pos={pos.id} {pos.symbol} 保本止损触发 entry={entry:.4f}")

                            # --- Mode 4: 时间止损 ---
                            elif _mode == 4:
                                max_minutes = int(pos.time_stop_minutes or 0)
                                if max_minutes > 0 and pos.entry_time:
                                    now_dt = datetime.now()
                                    if pos.entry_time.tzinfo:
                                        now_dt = now_dt.replace(tzinfo=pos.entry_time.tzinfo)
                                    held_minutes = (now_dt - pos.entry_time).total_seconds() / 60
                                    if held_minutes >= max_minutes:
                                        try:
                                            result = _close_position_for_risk(
                                                db, pos, acc, mark_price,
                                                tp_hit=False, sl_hit=False,
                                                drawdown_hit=False, daily_limit_hit=False,
                                                strategy=None, client=client,
                                                reason="time_stop",
                                            )
                                            ...
                                            logger.warning(f"[Trailing-TimeStop] pos={pos.id} 时间止损执行失败: {e}")
...
def _close_position_for_risk(
    ...
    *, tp_hit: bool, sl_hit: bool, drawdown_hit: bool, daily_limit_hit: bool,
    strategy, client=None, reason: str = "",
):
    from backend.exchanges.base import ExchangeClientBase
    ...
    if close_success:
        ...
        # 审计日志记录
        try:
            from backend.services.trade_audit import log_trade_event, snapshot_position, snapshot_strategy, snapshot_market
            audit_action = {
                "tp": TradeAuditLog.ACTION_RISK_TP,
                "sl": TradeAuditLog.ACTION_RISK_SL,
                "drawdown": TradeAuditLog.ACTION_RISK_DRAWDOWN,
                "daily_limit": TradeAuditLog.ACTION_RISK_DAILY,
                "time_stop": TradeAuditLog.ACTION_RISK_TIME_STOP,
                "risk": TradeAuditLog.ACTION_POSITION_CLOSED,
            }.get(reason, TradeAuditLog.ACTION_POSITION_CLOSED)
            log_trade_event(
                db,
                action_type=audit_action,
                action_desc=f"平仓-{reason}: {pos.symbol} {'多' if pos.side == 1 else '空'} pnl={realized:.4f}",
                user_id=pos.user_id,
                ...
                price=close_price,
                quantity=qty,
                market_snapshot=snapshot_market(close_price, pos.symbol),
                position_snapshot=snapshot_position(pos),
                strategy_snapshot=snapshot_strategy(strategy) if strategy else {},
                extra={"reason": reason, "realized_pnl": realized, "pnl_ratio": float(pos.pnl_ratio or 0)},
            )
        except Exception:
            pass
        ...
        # 分级告警推送
        try:
            from backend.utils.notifier import notify, SEVERITY_INFO, SEVERITY_WARNING, SEVERITY_CRITICAL
            if sl_hit or drawdown_hit or daily_limit_hit:
            ...
            notify(
                title=f"{pos.symbol} {reason.upper()} 平仓",
                detail=(
                    f"持仓ID: {pos.id}\n"
                    f"方向: {'多' if pos.side == 1 else '空'}\n"
                    f"开仓价: {entry:.4f}\n"
                    f"平仓价: {close_price:.4f}\n"
                    f"已实现盈亏: {realized:.4f} USDT ({pos.pnl_ratio:.2f}%)\n"
                    f"杠杆: {pos.leverage}x"
                ),
            ...
            logger.debug(f"[Notifier] 风控告警推送失败: {e}")

        # WebSocket 实时推送
        try:
            from backend.routers.websocket_channel import push_position_update, push_risk_alert
            push_position_update(pos.user_id, {
                "event": "position_closed",
                "symbol": pos.symbol,
                "side": "多" if pos.side == 1 else "空",
                "reason": reason,
                "pnl": round(realized, 4),
                "pnl_ratio": round(float(pos.pnl_ratio or 0), 2),
                "close_price": close_price,
            })
            push_risk_alert(pos.user_id, {
                "event": "risk_close",
                "symbol": pos.symbol,
                "reason": reason,
                "severity": alert_severity if 'alert_severity' in dir() else 2,
            })
        except Exception:
            pass
    if _close_client:
...
def _calc_position_atr(client, pos: TradePosition) -> float:
    """计算持仓品种的ATR值，用于ATR动态止损"""
    try:
        period = int(pos.atr_period or 14)
        if hasattr(client, 'fetch_klines'):
            candles = client.fetch_klines(symbol=pos.symbol, timeframe='4h', limit=period + 2)
        elif hasattr(client, 'get_klines'):
            candles = client.get_klines(symbol=pos.symbol, timeframe='4h', limit=period + 2)
        else:
            return 0.0
...
def _partial_close_position(db, pos: TradePosition, acc: ExchangeAccount,
                           close_price: float, partial_qty: float, client=None) -> bool:
    """分批平仓：按指定数量部分平仓"""
    from backend.exchanges.base import ExchangeClientBase
    from backend.exchanges._types import ORDER_TYPE_MARKET, SIDE_LONG, SIDE_SHORT
    _close_client = False
    ...
    try:
        client.place_order(
            ...
            leverage=pos.leverage,
            client_order_id=f"ptp_{pos.id}_{datetime.now().strftime('%H%M%S')}",
        )
    except Exception as e:
        logger.error(f"[PartialTP] 分批平仓API失败(pos={pos.id}): {e}")
        if _close_client:
            try:
                client.close()
            except Exception:
    ...
    qty = float(pos.quantity_contracts or 0)
    if pos.side == 1:
        realized = (close_price - entry) * partial_qty
    else:
        realized = (entry - close_price) * partial_qty

    pos.realized_pnl = Decimal(str(float(pos.realized_pnl or 0) + realized))
    pos.quantity_contracts = Decimal(str(max(0, qty - partial_qty)))
    pos.quantity_usdt = Decimal(str(float(pos.quantity_usdt or 0) * (1 - partial_qty / qty) if qty > 0 else 0))
    ...
    order = TradeOrder(
        ...
        symbol=pos.symbol,
        side=close_side,
        order_type=TradeOrder.TYPE_CLOSE,
        position_id=pos.id,
        leverage=pos.leverage,
        order_price=Decimal(str(close_price)),
        avg_fill_price=Decimal(str(close_price)),
        quantity_contracts=Decimal(str(partial_qty)),
        quantity_usdt=Decimal(str(partial_qty * close_price)),
        realized_pnl=Decimal(str(realized)),
        status=TradeOrder.STATUS_FILLED,
        trigger_reason=TradeOrder.REASON_TP,
    ...
    # 审计日志
    try:
        from backend.services.trade_audit import log_trade_event, snapshot_position
        log_trade_event(
            db,
            action_type=TradeAuditLog.ACTION_PARTIAL_CLOSED,
            action_desc=f"分批止盈: {pos.symbol} qty={partial_qty:.4f} pnl={realized:.4f}",
            user_id=pos.user_id,
            exchange_account_id=pos.exchange_account_id,
            strategy_id=pos.strategy_id,
            position_id=pos.id,
            ...
            quantity=partial_qty,
            position_snapshot=snapshot_position(pos),
            extra={"remaining_qty": float(pos.quantity_contracts), "realized_pnl": realized},
        )
    except Exception:
        pass
