...
def _calc_atr(closes, highs, lows, period=14):
    ...
    for i in range(1, len(closes)):
        ...
        trs.append(tr)
    atr_list = [0.0] * len(closes)
    for i in range(period, len(closes)):
        atr_list[i] = sum(trs[i - period:i]) / period
    return atr_list
...
def _run_composite_strategy(
    ...
    """7因子综合策略回测：简化版7因子加权评分，score > threshold 做多，score < -threshold 做空"""
    bt_leverage = int(params.get("leverage_fixed", 3) or 3)
    bt_risk_pct = float(params.get("single_position_ratio", 10) or 10) / 100
    bt_tp_pct = float(params.get("tp_ratio", 5.0) or 5.0) / 100
    bt_sl_pct = float(params.get("sl_ratio", 2.0) or 2.0) / 100
    threshold = float(params.get("score_threshold", 5.0) or 5.0) / 10.0

    sma_short = _calc_sma(closes, 7)
    sma_long = _calc_sma(closes, 25)
    rsi = _calc_rsi(closes, 14)
    bollinger = _calc_bollinger_bands(closes, 20, 2.0)
    atr = _calc_atr(closes, highs, lows, 14)
    macd_data = _calc_macd(closes, 12, 26, 9)
    ...
    for i in range(min_bars, len(closes)):
        ...
        # --- Factor 1: Market Regime (18%) ---
        regime_score = 0.0
        if i - 6 >= 0 and i - 24 >= 0 and len(sma_short) > i - 6 and len(sma_long) > i - 24:
            curr_s = sma_short[i - 6] if i - 6 < len(sma_short) else close
            curr_l = sma_long[i - 24] if i - 24 < len(sma_long) else close
            if curr_l > 0:
                ma_diff_pct = (curr_s - curr_l) / curr_l * 100
                regime_score = max(-10, min(10, ma_diff_pct * 2))

        # --- Factor 2: Capital Flow (15%) ---
        flow_score = 0.0
        if i >= 5 and volumes[i] > 0:
            avg_vol = sum(volumes[max(0, i-5):i]) / min(5, i) if i > 0 else 1
            vol_ratio = volumes[i] / avg_vol if avg_vol > 0 else 1
            price_change = (close - closes[i-1]) / closes[i-1] * 100 if closes[i-1] > 0 else 0
            flow_score = max(-10, min(10, price_change * vol_ratio * 2))

        # --- Factor 3: Leverage Concentration (12%) — 简化：用OI变化代理 ---
        leverage_score = 0.0  # 回测中无实时OI数据，权重由其他因子补

        # --- Factor 4: Liquidation Pressure (10%) — 简化：用ATR突变代理 ---
        liquidation_score = 0.0
        if atr[i] > 0 and i >= 14:
            avg_atr = sum(atr[max(0, i-14):i]) / min(14, i) if i > 0 else atr[i]
            if avg_atr > 0:
                atr_ratio = atr[i] / avg_atr
                if atr_ratio > 1.5:
        ...
        bb_idx = i - 19
        if bb_idx >= 0 and bb_idx < len(bollinger):
            bb = bollinger[bb_idx]
            if bb["bandwidth"] > 0:
                vol_pct = abs(close - bb["middle"]) / (bb["bandwidth"] * bb["middle"] / 2) * 100 if bb["bandwidth"] * bb["middle"] > 0 else 0
                if close > bb["upper"]:
                    volatility_score = min(10, vol_pct)
                elif close < bb["lower"]:
                    volatility_score = max(-10, -vol_pct)

        # --- Factor 6: News Sentiment (15%) — 回测中无新闻数据，留空 ---
        news_score = 0.0

        # --- Factor 7: Strategy Advantage (15%) — 简化：用MACD方向 ---
        strategy_score = 0.0
        if i < len(macd_data):
            macd = macd_data[i]
            if macd["histogram"] > 0:
                strategy_score = min(10, macd["histogram"] / abs(close) * 1000 * 10) if close > 0 else 0
            else:
                strategy_score = max(-10, macd["histogram"] / abs(close) * 1000 * 10) if close > 0 else 0
        ...
        if not position:
            if composite > threshold and curr_rsi < 75:
                entry_price = close * (1 + slippage)
                leverage = bt_leverage
                margin = capital * bt_risk_pct
                if margin <= 0:
                ...
                position = {
                    "side": 1, "entry_price": entry_price, "entry_idx": i,
                    "quantity": quantity, "leverage": leverage, "margin": margin,
                    "fee": fee, "entry_time": timestamps[i], "entry_score": composite,
                    "tp_price": entry_price * (1 + bt_tp_pct),
                    "sl_price": entry_price * (1 - bt_sl_pct),
                }
                capital -= (margin + fee)
            elif composite < -threshold and curr_rsi > 25:
                entry_price = close * (1 - slippage)
                leverage = bt_leverage
                margin = capital * bt_risk_pct
                if margin <= 0:
        ...
        if position:
            ...
            if should_close:
                ...
                trades.append({
                    ...
                    "exit_time": timestamps[i],
                    "holding_bars": i - position["entry_idx"],
                    "entry_score": round(position.get("entry_score", 0), 2),
                })
                position = None
    ...
    try:
        ...
        for sym_idx, symbol in enumerate(symbols):
            ...
            try:
                ...
                if _strat_type == "composite":
                    capital = initial_capital
                    sym_trades, capital = _run_composite_strategy(
                        closes, highs, lows, volumes, timestamps, klines_data,
                        capital, slippage, fee_rate, _params, symbol, timeframe,
                    )
                    all_trades.extend(sym_trades)
...
def _generate_mock_klines(symbol, timeframe, date_start, date_end):
    ...
    base_prices = {"BTC": 65000, "ETH": 3200, "SOL": 150, "XRP": 0.5, "BNB": 500,
                   ...
                   "XAG": 30, "TSLA": 250, "NVDA": 120, "AAPL": 220, "MSFT": 420,
                   "TCEHY": 60, "GOOGL": 165, "AMZN": 185, "META": 500, "NFLX": 650,
                   "KO": 62, "PG": 168, "WMT": 82, "JNJ": 160, "PEP": 175,
                   "INTC": 30, "MCD": 295, "JPM": 210, "AMD": 145, "MSTR": 1300,
                   "COIN": 180, "PLTR": 32, "SMCI": 450, "SKHYNIX": 1219, "SNDK": 1555,
                   }
